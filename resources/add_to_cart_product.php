    ...
    if(!isset($_SESSION['userid'])){
        ...
        if($check_cart->rowCount() > 0){
            ...
            if($feproduct['stock'] >= $product_quentity){
                ...
                $update = $db->query("UPDATE phtv_product_cart SET qty = '$product_quentity', amount = '$product_price', total_amount = '$total_amount' WHERE id = '$cart_id'");
                if($update){
            ...
            if($feproduct['stock'] >= $product_quentity){
                $product_price = $feproduct['price'];
                $total_amount = $product_price * $product_quentity;
                $update = $db->query("INSERT INTO phtv_product_cart SET product_id = '$product_id', user_id = '$user_id', color_id = '$product_color_id', size_id = '$product_size_id', qty = '$product_quentity', amount = '$product_price', total_amount = '$total_amount'");
                if($update){
                    $success['success'] = "success";
